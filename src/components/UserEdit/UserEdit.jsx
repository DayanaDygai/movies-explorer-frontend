...
import { EMAIL_VALIDATION_REGEX, NAME_VALIDATION_REGEX, messageKeys } from '../../utils/constants';


// Компонент для редактирования данных пользователя
function UserEdit({ handleUpdateUser, handleLogout, isLoading }) {
  const { isFormValid, errors, fieldsValidity, setFieldsValidity, formValues, resetForm, setFormValues, validateInputField } =
  useFormValidation();
  ...
  return (
    ...
    >
      ...
      <Input
        ...
        maxLength="40"
        value={name || ''}
        handleChangeValidation={(e) => validateInputField(e, NAME_VALIDATION_REGEX, messageKeys.NAME_FORMAT_ERROR)}
        errors={errors}
      ...
      fieldsValidity={fieldsValidity}
        ...
        maxLength="40"
        value={email || ''}
        handleChangeValidation={(e) => validateInputField(e, EMAIL_VALIDATION_REGEX, messageKeys.EMAIL_FORMAT_ERROR)}
        errors={errors}
        className="input input_type_useredit"
        addClassName="input_display_block"
        required
